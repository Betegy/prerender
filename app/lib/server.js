...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.run(_this.options.blockedResources || blockedResources, !!_this.options.logRequests, req.headers['user-agent'], function(blockedResources, logRequests, userAgent, resolve, reject) {
        ...
        this.settings.userAgent = userAgent + ' Prerender (+https://github.com/prerender/prerender)';
...
server.checkIfPageIsDoneLoading = function(req, res) {
    ...
    if(req.prerender.isPageClosed) {
        console.log('PhantomJS page was closed in the middle of this request. Aborting...')
        clearInterval(req.prerender.downloadChecker);
        req.prerender.downloadChecker = null;
        return res.send(504);
    }
